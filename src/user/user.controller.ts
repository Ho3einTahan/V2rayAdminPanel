...
export class UserController {
    ...
    async updateUserData(@Body() body, @Res() res: Response, @Req() req: Request) {
        const { userName, password, startServiceDate, macAddress, endServiceDate, phoneNumber, originalPhoneNumber, status, role, multiUser, serviceType } = body;

        try {
            await this.userService.updateUserByPhoneNumber(role, originalPhoneNumber, userName, password, macAddress, startServiceDate, endServiceDate, phoneNumber, status, multiUser, serviceType);
